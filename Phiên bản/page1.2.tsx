...
function createBossEvent(sourceGuilds: Guild[], sourceStudents: Student[], sourceQuestions: Question[]): BossEvent {
  ...
  const element = ELEMENTS[totalPower % ELEMENTS.length];
  return {
    id: Date.now(),
    name: "Boss thế giới",
    ...
    questionIds: getBossQuestionIds(sourceQuestions),
          ...
          <div style={styles.card}>
            ...
            {bossEvent ? (
              <>
                ...
                <div>Boss dùng ảnh: <b>{getBossImage()}</b></div>
...
Hình: https://...`}
            ...
            <div style={{ ...styles.card }}>
              <h3>Ngân hàng câu hỏi theo lớp và chủ đề ({filteredQuestions.length})</h3>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {groupedQuestionBank.map((classGroup) => (
                  <details key={classGroup.className} open style={{ border: "1px solid #e2e8f0", borderRadius: 16, background: "#fff", padding: 12 }}>
                    <summary style={{ cursor: "pointer", fontWeight: 800, fontSize: 18 }}>
                      Lớp {classGroup.className} · {classGroup.topics.reduce((sum, topic) => sum + topic.items.length, 0)} câu
                    </summary>
                    <div style={{ display: "grid", gap: 12, marginTop: 12 }}>
                      {classGroup.topics.map((topicGroup) => (
                        <details key={`${classGroup.className}-${topicGroup.topic}`} open={questionFilterGroup !== "Tất cả"} style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 12, background: "#f8fafc" }}>
                          <summary style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                            <span style={{ fontWeight: 700 }}>Chủ đề: {topicGroup.topic} · {topicGroup.items.length} câu</span>
                            <button
                              type="button"
                              style={styles.secondaryBtn}
                              onClick={(e) => {
                                e.preventDefault();
                                jumpToQuestionForm(classGroup.className === "Chưa phân lớp" ? "" : classGroup.className, topicGroup.topic);
                              }}
                            >
                              Mở chủ đề và thêm câu hỏi
                            </button>
                          </summary>
                          <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                            {topicGroup.items.map((q) => (
                              <div key={q.id} style={styles.miniCard}>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                                  <div style={{ flex: 1, minWidth: 260 }}>
                                    <div><b>{q.question}</b></div>
                                    <div style={{ marginTop: 4, color: "#475569" }}>Mức độ: {q.difficulty} · Lớp {q.className || "Chưa phân lớp"} · Chủ đề {q.group} · Đáp án đúng: {q.correctAnswer}</div>
                ...
                {!groupedQuestionBank.length && <div>Chưa có câu hỏi nào khớp bộ lọc hiện tại.</div>}
              </div>
            </div>
          </div>
        )}
