...
function createBossEvent(
  ...
  const questionIds = options?.questionIds?.length ? options.questionIds : getBossQuestionIds(sourceQuestions);
  return {
    id: Date.now(),
    ...
    questionIds,
    description: options?.description || "Boss đã xuất hiện. Học sinh vào làm nhiệm vụ để gây sát thương cho boss.",
...
export default function Page() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (canSpawnBoss && bossSpawnGateOpen && !bossEvent?.resolvedAt && !bossEvent) {
      const newBoss = createBossEvent(guilds, students, questions);
      setBossEvent(newBoss);
      setBossSpawnGateOpen(false);
      addLog("boss_spawn", `Boss thế giới đã xuất hiện với ${newBoss.maxHp} HP vì toàn bộ quân đoàn đều đã có thú.`);
    }
  ...
  function summonBossManually() {
    if (!canSpawnBoss) return alert("Chưa đủ điều kiện triệu hồi boss. Tất cả quân đoàn đang hoạt động phải có thú và phải có câu hỏi.");
    if (bossEvent && !bossEvent.resolvedAt) return alert("Đã có boss đang hoạt động.");
    const questionIds = getBossQuestionIdsByClassAndGroup(questions, bossTaskClassFilter, bossTaskGroupFilter);
    const nextBoss = createBossEvent(guilds, students, questions, {
      element: bossElementInput,
      questionIds: questionIds.length ? questionIds : getBossQuestionIds(questions),
      description: bossDescriptionInput.trim() || "Boss đã xuất hiện. Học sinh vào làm nhiệm vụ để gây sát thương cho boss.",
    });
    setBossEvent(nextBoss);
    setBossSpawnGateOpen(false);
    addLog("boss_spawn_manual", `Giáo viên triệu hồi boss hệ ${nextBoss.element} với ${nextBoss.questionIds.length} câu hỏi.`);
  }
  ...
  function appendBossTaskQuestions() {
    ...
    if (!pickedIds.length) return alert("Bộ lọc hiện tại chưa có câu hỏi nào để thêm.");
    const mergedIds = Array.from(new Set([...(bossEvent.questionIds || []), ...pickedIds]));
    setBossEvent((prev) => prev ? { ...prev, questionIds: mergedIds, description: bossDescriptionInput.trim() || prev.description } : prev);
    addLog("boss_task_append", `Đã thêm ${mergedIds.length - (bossEvent.questionIds || []).length} câu hỏi vào nhiệm vụ boss đang sống.`);
  }
            ...
            {bossEvent ? (
              <>
                ...
                <div style={{ marginTop: 10, display: "flex", justifyContent: "center" }}>
                  <img src={getBossImage()} alt="boss" style={{ width: 180, height: 180, objectFit: "contain", borderRadius: 18, border: "1px solid #e2e8f0", background: "#fff7ed", padding: 10 }} />
                </div>
                ...
                <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 12, background: "#fff7ed", border: "1px solid #fed7aa" }}>
                  <b>Nội dung nhiệm vụ:</b> {bossEvent.description || "Chưa có mô tả thêm từ giáo viên."}
                </div>
                ...
                {!bossEvent.resolvedAt && !getBossParticipantContribution(bossEvent, currentStudent.id) && activeBossBattleId !== bossEvent.id && currentStudent.beast && (
                ...
                <div style={styles.miniCard}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", color: "#64748b", fontSize: 13 }}>
                      Bộ lọc hiện tại có {getBossQuestionIdsByClassAndGroup(questions, bossTaskClassFilter, bossTaskGroupFilter).length} câu hỏi.
                    </div>
                  </div>
                  <textarea style={styles.textarea} value={bossDescriptionInput} onChange={(e) => setBossDescriptionInput(e.target.value)} placeholder="Nội dung / yêu cầu thêm của nhiệm vụ boss" />
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    <button style={styles.primaryBtn} onClick={summonBossManually}>Triệu hồi boss mới</button>
                    <button style={styles.secondaryBtn} onClick={updateBossDescription}>Cập nhật nội dung boss đang sống</button>
                    <button style={styles.secondaryBtn} onClick={appendBossTaskQuestions}>Thêm câu hỏi cho boss đang sống</button>
                  </div>
                </div>
              {bossEvent ? (
                <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
                  ...
                  <div>Số câu hỏi hiện tại: {bossEvent.questionIds.length}</div>
                  <div style={{ marginTop: 10, display: "flex", justifyContent: "center" }}>
                    <img src={getBossImage()} alt="boss" style={{ width: 220, height: 220, objectFit: "contain", borderRadius: 18, border: "1px solid #e2e8f0", background: "#fff7ed", padding: 10 }} />
                  </div>
                  <div>Ảnh boss: {getBossImage()}</div>
                  <div style={{ padding: "10px 12px", borderRadius: 12, background: "#fff7ed", border: "1px solid #fed7aa" }}>
                    <b>Nội dung nhiệm vụ:</b> {bossEvent.description || "Chưa có mô tả thêm."}
                  </div>
